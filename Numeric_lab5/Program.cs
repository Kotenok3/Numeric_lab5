...
namespace Numeric_lab5
{
    class Program
    {
        ...
        //public static double F(double x, double t) => 3 * t * t;
        ...
        public static void RightProgonk(int N, int M, double G)
        {
            ...
            Console.WriteLine($"n = {N}, m = {M}, G = {G}, max eps = {Enumerable.Range(0,M+1).Select(i => Math.Abs(Uxt(i*h,1)-Y[i])).Max():f4}" );
            Console.WriteLine("  m |   U[m]  |    y[m]   | eps");
            for (int i = 0; i <= M; i++)
            {
